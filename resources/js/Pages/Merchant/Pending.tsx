...
import { FiAlertTriangle, FiCheckCircle, FiLock, FiXCircle } from 'react-icons/fi';
import AuthStorefrontLayout from '@/layouts/AuthStorefrontLayout';
import type { PageProps, User } from '@/types';

export default function Pending() {
    const { auth, company } = usePage<PageProps<{ company?: { status?: string } }>>().props;
    const user = auth.user as User;
    const status = company?.status || user.status || 'pending';
                        ...
                        <div className="flex flex-col items-center gap-6 text-center">
                            ...
                            <p className="max-w-2xl text-base leading-7 text-slate-600">
                                {isRejected
                                    ...
                                    : 'Votre compte commerçant est en attente de validation par l’administrateur. Vous serez informé dès que votre entreprise sera validée.'}
                            </p>
                            <div className="grid w-full gap-4 sm:grid-cols-2">
                                ...
                                <div className="rounded-3xl border border-black/10 bg-slate-50 p-6 text-left">
                                    ...
                                    <p className="mt-3 text-base text-slate-700">
                                        {isRejected ? 'Vérifiez votre dossier et contactez le service client.' : 'Un administrateur examine votre dossier et vous répondra bientôt.'}
                                    </p>
                                </div>
                            </div>
                        </div>
